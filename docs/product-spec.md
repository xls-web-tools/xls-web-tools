...
- 値式。`ValueExpression` として、他の抽出列または派生列の値から派生列の値を計算する式を指定する。通常の抽出列では空欄にする。
...
- `ValueExpression` は文字列値を返す式とする。単純な別名は `[列1]` のように列参照だけで表し、条件付き値は `IF([列2] == "", [列1], [列2])` の形式で表す。`IF` の条件部分では、`OutputConditionExpression` と同じく `[OutputColumnName]` 形式の列参照、`AND` / `OR` / `NOT`、`==` / `!=`、ダブルクォート文字列リテラル、同じエスケープシーケンスを使う。
...
条件式の評価は、詳細ページ列定義のうち条件式に必要な抽出列を取得し、条件式に必要な派生列を計算した後、必須項目エラー判定より先に行う。条件式に必要な値が取れて条件不一致になった場合は、他の必須列が空でも出力対象外とし、`ERROR` 行を作らない。条件式で参照している抽出列自体を selector 欠落、frame 欠落、attribute 欠落などで読めない場合は、その抽出値を空文字列として条件判定する。ただし WebDriver session や script 実行そのものの失敗は、従来どおり詳細ページ単位の `ERROR` とする。条件が true になった後の通常の詳細列取得では、出力列として選ばれているかどうかに関係なく、元の `IsRequired` と `BlankMode` に従う。
条件式で参照できる列は、詳細ページ列定義の `OutputColumnName` に限定する。列参照の照合は大文字小文字を区別する。出力列として選ばれていない抽出列や派生列も条件式から参照できる。固定管理列の `対象ID`、`取得状態`、`エラー内容`、`ダウンロード状態` 自体は条件式から参照しない。同名の `OutputColumnName` を持つ詳細ページ列定義がある場合、`[対象ID]` などの列参照は固定管理列ではなく、その抽出列または派生列を参照する。
