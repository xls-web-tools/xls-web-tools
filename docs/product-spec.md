...
- 必須かどうか。
- 空欄時の扱い。
- output シートへ出力するかどうか。設定列名は `OutputEnabled` とし、既存の `BlankMode` の右隣に置く。`True` / `False` / `1` / `0` / `Yes` / `No` を指定でき、空欄または列未定義の場合は `False` として扱う。詳細ページ列定義は全候補列が空欄の行だけを空行として無視する。
...
固定管理列の後ろに、settings の詳細ページ列定義のうち `OutputEnabled=True` の抽出列だけを、詳細ページ列定義の順序を保って左詰めに並べる。`OutputEnabled=False` の抽出列用の空列は作らない。`OutputEnabled=False` の抽出列とダウンロードファイルパスは output シートへ出力しない。すべての詳細ページ列が `OutputEnabled=False` の場合も有効な設定とし、output シートには固定管理列だけを出力する。`OutputEnabled=False` の抽出列が必須列エラーなどの原因になった場合も、その列自体は出力せず、固定管理列の `取得状態=ERROR` と `エラー内容` で原因を示す。`OutputEnabled` は output シートへ記録するかだけを制御し、抽出自体は省略しない。現在の出力幅より右側に残る古いヘッダーや値は、ツール管理領域として削除してよい。
...
条件式の評価は、詳細ページ列定義の抽出値を取得した後、必須項目エラー判定より先に行う。条件式に必要な値が取れて条件不一致になった場合は、他の必須列が空でも出力対象外とし、`ERROR` 行を作らない。条件式で参照している列自体を selector 欠落、frame 欠落、attribute 欠落などで読めない場合は、その列の抽出値を空文字列として条件判定する。ただし WebDriver session や script 実行そのものの失敗は、従来どおり詳細ページ単位の `ERROR` とする。条件が true になった後の通常の詳細列取得では、`OutputEnabled` の値に関係なく、元の `IsRequired` と `BlankMode` に従う。
条件式で参照できる列は、詳細ページ列定義の `OutputColumnName` に限定する。`OutputEnabled=False` の抽出列も条件式から参照できる。固定管理列の `対象ID`、`取得状態`、`エラー内容` は条件式から参照しない。
...
詳細ページ列定義で同じ `OutputColumnName` が複数定義されている場合は、`OutputConditionExpression` の有無や `OutputEnabled` の値に関係なく settings エラーとする。`OutputEnabled=False` の抽出列でも `OutputColumnName` は必須とする。`OutputColumnName` が固定管理列名の `対象ID`、`取得状態`、`エラー内容`、`ダウンロード状態` と一致する場合も、`OutputEnabled` の値に関係なく settings エラーとする。
条件式が不正な場合は、収集開始前の settings エラーとして中断する。未定義の `[OutputColumnName]`、括弧不整合、演算子不足、未知のエスケープシーケンスは、詳細ページ処理中の ERROR 行ではなく設定誤りとして扱う。`OutputEnabled` に有効値以外が指定されている場合も、収集開始前の settings エラーとして中断する。`OutputEnabled` だけが入力され、`OutputColumnName` や `Selector` が空の行も settings エラーとして中断する。
...
診断モードの出力先は本番収集と同じ出力先シートとし、`OutputEnabled` による列構成も本番収集と同じにする。
...
収集開始時は、output シートの固定管理列と `OutputEnabled=True` の出力列を現在の列構成へ整える。既存の出力列値はヘッダー名で現在の出力列へ対応付け、現在の出力列に存在しない古い列は削除する。現在の出力列ではない古いヘッダーが複数ある場合も、出力シート不整合とはせず削除対象とする。新しく増えた出力列は空欄のままとし、既存 OK 行の値を埋める必要がある場合は `Overwrite` で再取得する。既存 output シートに現在の出力列名と同じヘッダーが複数ある場合は、どの値を正とするか決められないため、出力シート不整合として中断する。
