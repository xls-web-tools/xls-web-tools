...
settings シートでは C 列を利用者メモ、D 列を空列、画面遷移操作定義を E:J、K 列を空列、詳細ページ列定義を L:R に置く。旧 workbook レイアウトとの互換は対象外とし、テンプレートと workbook 側の列構成を更新して使う。
...
詳細ページ列定義テーブルの列順は `OutputColumnName`, `Selector`, `ExtractType`, `AttributeName`, `IsRequired`, `ValueExpression`, `BlankMode` とする。`ValueExpression` 列がない旧形式の workbook との互換は対象外とし、テンプレートと workbook 側の列構成を更新して使う。
...
- attribute 名。
- 必須かどうか。
- 値式。`ValueExpression` として、`IsRequired` と `BlankMode` の間の列に指定し、他の抽出列または派生列の値から派生列の値を計算する式を指定する。通常の抽出列では空欄にする。
- 空欄時の扱い。
- 詳細ページ列定義は全列が空欄の行だけを空行として無視する。`OutputColumnName` は必須とする。通常の抽出列は `Selector` を必須とし、`ValueExpression` を空欄にする。派生列は `ValueExpression` を必須とし、`Selector` を空欄にする。`Selector` と `ValueExpression` が両方空欄、または両方非空の非空行は settings エラーとして扱う。
...
- `ValueExpression` の戻り値は、参照元の列値、文字列リテラル、または `IF` の分岐結果をそのまま返す。派生値の計算結果に対して追加の `Trim` は行わない。
