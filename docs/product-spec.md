...
3. `ListTransitionOperationName` の画面遷移操作で一覧画面へ進み、その操作行の `WaitSelector` で一覧到達を確認する。一覧画面と認証後の起点ページが同一になる場合は、`ActionScript=return true;` のような副作用のない script-only 操作を設定する。
4. 一覧画面上の一覧項目を表示順に扱う。
5. 一覧項目の詳細遷移操作を WebDriver の click または `ActionScript` で再現し、その操作行の `WaitSelector` で詳細ページ到達を確認する。
6. 詳細ページで対象IDを取得し、設定された列定義に基づいて出力行を作る。
7. 一覧復帰リンクの画面遷移操作で一覧画面へ戻り、その操作行の `WaitSelector` で一覧到達を確認する。
...
- `ListTransitionOperationName`: 一覧画面へ進む画面遷移操作名。必須。
...
- `DetailTransitionOperationName`: 一覧項目から詳細ページへ入る画面遷移操作名。必須。
- `TargetIdSelector`: 詳細ページ上の対象IDを読み取る selector。
- `ReturnToListOperationName`: 詳細ページから一覧画面へ戻る画面遷移操作名。診断モードと本番収集の両方で必須とする。
- `NextPageOperationName`: 現在ページの処理後に次ページへ進む画面遷移操作名。空欄の場合はページングしない。指定する場合は `NextPageAvailableSelector` も必須。
- `NextPageAvailableSelector`: 次ページ操作前に、次ページ操作が有効な場合だけ一致する selector。空欄の場合はページングしない。指定する場合は `NextPageOperationName` も必須。0 件の場合は次ページなしとして正常終了する。
- `OutputSheetName`: 取得結果の出力先シート名。
- `ExistingRowMode`: 再実行時の既存行扱い。既定値は `SkipExisting`。有効値は `SkipExisting`、`Overwrite`、`FillBlankCells` とする。`SkipExisting` は既存 OK 行だけをスキップし、未登録行と既存 ERROR 行を取得する。`Overwrite` は既存状態に関係なく全更新する。`FillBlankCells` は既存行を保持し、有効な出力列に本当に空白のセルがある場合だけ再取得して、本当に空白かつ今回の出力値が空文字列ではないセルだけを補完する。`RetryError` は旧称であり、有効な設定値として扱わない。
...
- `ActionScript`: JavaScript 関数名または script 本文。click が難しい場合、または画面遷移が不要な場合に使う。
- `WaitSelector`: 操作後に遷移先画面へ到達したことを判定する CSS selector。

画面遷移操作は `OperationName` で参照する。`ActionSelector` がある場合は CSS selector click を先に実行し、click が失敗して `ActionScript` が指定されている場合だけ script 実行へ切り替える。`ActionSelector` が空で `ActionScript` が指定されている場合は script だけを実行する。`OperationName` と `WaitSelector` は必須であり、`ActionSelector` または `ActionScript` の少なくとも一方を指定する。`ActionInnerText` を指定する場合は `ActionSelector` も必須とする。`OperationName` の照合は大文字小文字を区別しない。`LocatorType` は廃止し、操作対象 selector は CSS selector 固定とする。

遷移不要で一覧到達だけ確認したい場合は、`ActionSelector` を空欄にし、`ActionScript` に `return true;` のような副作用のない script を指定し、`WaitSelector` に一覧到達 selector を指定する。

`ActionInnerText` を指定した場合は、`ActionSelector` に一致する候補要素の DOM `innerText` を読み取り、前後空白、改行、連続空白を正規化した文字列で完全一致する要素だけを操作対象とする。大文字小文字、全角半角、かなは区別する。一致件数が 0 件または 2 件以上の場合は操作対象を確定できないためエラーとし、`ActionScript` fallback へは進まない。1 件に確定した後の click 失敗は、既存どおり `ActionScript` が指定されている場合だけ script 実行へ切り替える。対象未確定エラーには `OperationName`、`ActionSelector`、`ActionInnerText`、一致件数を含める。

CSS selector で iframe / frame 内の要素を指定する場合は、`frame selector >> target selector` の形式を `ActionSelector` または `WaitSelector` に書く。`>>` で区切った途中要素は frame selector、最後の要素は target selector として扱う。frame selector と target selector は空にできない。`ActionInnerText` は最後の target selector に一致する候補要素へ適用する。具体的な selector 値は取得対象の環境に依存するため、ドキュメントや配布テンプレートには実サイトの値を残さず、利用者が settings に入力する。settings テンプレートには `OpenList`、`OpenDetail`、`ReturnToList`、`NextPage` の操作名だけを用意し、selector 値、`ActionInnerText`、`ActionScript`、`WaitSelector` は空欄にする。

本番収集で一覧項目ごとに変わる `ActionSelector` / `ActionScript` / `WaitSelector` には、0-based の `{{index}}` と 1-based の `{{rowNumber}}` を使える。`{{index}}` は `querySelectorAll(...)[{{index}}]` などに使い、`{{rowNumber}}` は CSS の `nth-child({{rowNumber}})` などに使う。
...
- selector。通常の抽出列でだけ指定し、派生列では空欄にする。
- 取得種別。`TextContent`、`InnerText`、`Attribute` など。
- attribute 名。
- 必須かどうか。
- 値式。`ValueExpression` として、`IsRequired` と `OutputValueType` の間の列に指定し、他の抽出列または派生列の値から派生列の値を計算する式を指定する。通常の抽出列では空欄にする。
- 出力値型変換方針。`OutputValueType` として `String` または `Auto` を指定する。空欄は `String` として扱う。
- 空欄時の扱い。
- 詳細ページ列定義は全列が空欄の行だけを空行として無視する。`OutputColumnName` は必須とする。通常の抽出列は `Selector` を必須とし、`ValueExpression` を空欄にする。派生列は `ValueExpression` を必須とし、`Selector` を空欄にする。`Selector` と `ValueExpression` が両方空欄、または両方非空の非空行は settings エラーとして扱う。`OutputValueType` の指定値は、出力列として選ばれているかどうかに関係なく、すべての詳細ページ列定義で検証する。`String` / `Auto` 以外の値は大文字小文字の差だけを許容し、それ以外は settings エラーとして扱う。
- `ValueExpression` は文字列値を返す式とする。使える値式は、`[列1]` のような列参照、ダブルクォート文字列リテラル、`IF(条件, 値式, 値式)` に限定する。単純な別名は `[列1]` のように列参照だけで表し、条件付き値は `IF([列2] == "", [列1], [列2])` の形式で表す。`IF` の条件部分では、`OutputConditionExpression` と同じく `[OutputColumnName]` 形式の列参照、`AND` / `OR` / `NOT`、`==` / `!=`、ダブルクォート文字列リテラル、同じエスケープシーケンスを使う。文字列結合や `COALESCE` / `TRIM` などの未対応関数は settings エラーとして扱う。
- `ValueExpression` の戻り値は、参照元の列値、文字列リテラル、または `IF` の分岐結果をそのまま返す。派生値の計算結果に対して追加の `Trim` は行わない。`OutputValueType` は `ValueExpression` の評価結果や `OutputConditionExpression` の判定には影響せず、output シートへ値を書き込む時だけ適用する。
- 派生列は settings の行順に関係なく、他の抽出列または派生列を参照できる。評価順は `ValueExpression` の依存関係から決める。未定義の `[OutputColumnName]` 参照、自己参照、派生列同士の循環参照は収集開始前の settings エラーとして扱う。
- 派生列で `IsRequired=True` が指定されている場合は settings エラーとして扱う。派生列の `BlankMode=ErrorIfBlank` は、出力対象条件に一致した後、`ValueExpression` の結果が空文字列の場合に詳細ページ単位の `ERROR` とする指定として扱う。
...
固定管理列の後ろでは、output シート 1 行目の E 列以降について、空文字列を無視した 1 行目の used range を取得し、その範囲内の非空ヘッダーのうち、settings の詳細ページ列定義に存在する `OutputColumnName` と大文字小文字を区別して一致する列を出力列として扱う。E 列以降の非空ヘッダーの並び順を出力列順とし、settings の詳細ページ列定義に存在しても非空ヘッダーに指定されていない抽出列または派生列は output シートへ出力しない。E 列以降に出力列がない場合も有効な設定とし、output シートには固定管理列だけを出力する。1 行目が空白の列、または settings の詳細ページ列定義に存在しない未定義ヘッダー列は処理対象外とし、値も数式も入っていない場合は空のまま維持し、値または数式が入っている場合も変更しない。非空ヘッダーが重複している場合は、同じ値を該当するすべての列へ、該当する詳細ページ列定義の `OutputValueType` で出力する。`OutputValueType=String` の場合は文字列として書き込み、`OutputValueType=Auto` の場合は Excel エラー文字列、数値、Boolean、日付、Currency として解釈できる値をワークシート上の値として書き込み、いずれにも解釈できない値を文字列として書き込む。`Auto` はセルの表示形式を自動設定しない。空欄は型変換対象外とし、`String` / `Auto` のどちらでも空欄として書き込む。固定管理列は `OutputValueType` の対象外とする。非空ヘッダーが固定管理列名と同じ場合も、固定管理列と E 列以降の出力列の両方へ出力する。出力列ではない抽出列または派生列が必須列エラーなどの原因になった場合も、その列自体は出力せず、固定管理列の `取得状態=ERROR` と `エラー内容` で原因を示す。抽出と派生値計算は出力列指定の有無だけでは省略しない。固定管理列の 1 行目 (A1:D1) は利用者が変更できる表示名として扱う。A1:D1 のセルが本当に空白の場合だけ既定ヘッダーを書き込み、値、数式、または空文字列が入っている場合は検証も上書きもしない。E 列以降の出力列ヘッダーは settings から自動生成しない。
...
条件式の評価は、詳細ページ列定義のうち条件式に必要な抽出列を取得し、条件式に必要な派生列を計算した後、必須項目エラー判定より先に行う。条件式に必要な値が取れて条件不一致になった場合は、他の必須列が空でも出力対象外とし、`ERROR` 行を作らない。条件式で参照している抽出列自体を selector 欠落、frame 欠落、attribute 欠落などで読めない場合は、その抽出値を空文字列として条件判定する。ただし WebDriver session や script 実行そのものの失敗は、従来どおり詳細ページ単位の `ERROR` とする。条件が true になった後の通常の詳細列取得では、出力列として選ばれているかどうかに関係なく、通常の抽出列は元の `IsRequired` と `BlankMode` に従い、派生列は `BlankMode` に従う。
条件式で参照できる列は、詳細ページ列定義の `OutputColumnName` に限定する。列参照の照合は大文字小文字と LF を区別する。出力列として選ばれていない抽出列や派生列も条件式から参照できる。列参照内の LF は実 LF で記述することを正式構文とし、`\n` は列参照ではなく文字列リテラル用のエスケープとして扱う。固定管理列の `対象ID`、`取得状態`、`エラー内容`、`ダウンロード状態` 自体は条件式から参照しない。同名の `OutputColumnName` を持つ詳細ページ列定義がある場合、`[対象ID]` などの列参照は固定管理列ではなく、その抽出列または派生列を参照する。
文字列比較は case-sensitive とする。比較時は、列値と式内の文字列リテラルの前後空白を除去してから `==` または `!=` を評価する。
...
既存行がある対象が出力対象外になった場合は、`ExistingRowMode` が `Overwrite` または `FillBlankCells` であっても既存行を残し、更新も削除もしない。既存 `取得状態=ERROR` の行を `SkipExisting` で再取得した結果、または空白セルがある既存行を `FillBlankCells` で再取得した結果、出力対象条件に一致しなかった場合も、既存行を更新も削除もせず残し、その実行では出力対象外件数として数える。
...
診断モードの出力先は本番収集と同じ出力先シートとし、output シートのヘッダーによる列構成と `OutputValueType` による出力値型変換方針も本番収集と同じにする。詳細ページ列定義が 0 件の場合も、固定管理列だけの診断出力とダウンロード確認を有効とする。
...
現在ページの未処理項目がなくなったら `NextPageAvailableSelector` で次ページ操作の有効性を確認し、有効な場合だけ `NextPageOperationName` の画面遷移操作を実行し、その操作行の `WaitSelector` で一覧到達を確認する。`NextPageOperationName` と `NextPageAvailableSelector` が両方空欄の場合はページングなしとして正常終了し、片方だけ指定されている場合は settings エラーにする。`NextPageAvailableSelector` が 0 件の場合は次ページなしとして正常終了する。
全ページ収集の完了メッセージでは、正常件数、スキップ件数、エラー件数に加えて、処理ページ数を表示する。スキップ件数は `SkipExisting` により再取得しなかった既存 OK 行と、`FillBlankCells` で有効な出力列に本当に空白のセルがないため再取得しなかった既存行を数える。既存 ERROR 行を `SkipExisting` で再取得した件、または既存空白セルを `FillBlankCells` で補完した件は、再取得後の結果に応じて正常件数またはエラー件数に数える。初回ページが 0 件でも、一覧画面に到達して確認した実行事実として処理ページ数は 1 とする。
収集準備進捗は、`Collect_Click` と `RunDiagnostic_Click` で WebDriver session 開始から一覧画面到達まで表示する。表示名は `取得準備中` 固定とし、WebDriver session 開始完了、取得開始 URL 遷移完了、認証後の起点ページ到達、一覧画面遷移操作完了、一覧画面到達の 5 段階を固定ステップとして扱う。準備完了時または準備中にエラーで中断する場合は、ステータスバーの準備表示を解除する。

収集進捗は、`Collect_Click` の本番収集で一覧項目数が判明してから表示する。表示名は `取得中` 固定とする。初回ページで一覧項目が 1 件以上あることを確認してから表示し、開始時は 0% をステータスバーへ表示する。終了値はページ到達ごとに判明した一覧項目数を加算した値とし、新しいページの一覧項目数を加算した結果、表示パーセントが下がることを許容する。進捗値は一覧項目の処理完了数とし、詳細ページ処理後に必要な一覧復帰まで完了した時点で 1 件進める。既存 OK 行としてスキップした一覧項目、出力対象外になった一覧項目、ERROR 行として出力した一覧項目も処理完了数に含める。初回ページの一覧項目が 0 件の場合は収集進捗を開始せず、正常終了の完了メッセージだけを表示する。収集中にエラーで中断する場合は、完了扱いにせず、ステータスバーの進捗表示を解除してからエラーメッセージを表示する。
...
収集開始時も、output シートの A:D は列位置で固定管理列として扱い、A1:D1 は利用者が変更できる表示名として扱う。A1:D1 のセルが本当に空白の場合だけ既定ヘッダーを書き込み、値、数式、または空文字列が入っている場合は検証も上書きもしない。E 列以降は空文字列を無視した 1 行目の used range 内の非空ヘッダーを読む。settings の詳細ページ列定義に存在する `OutputColumnName` と大文字小文字を区別して一致するヘッダーだけを出力列として扱い、各出力列ヘッダーが置かれた列へ `OutputValueType` に従って書き込む。空白ヘッダー列と未定義ヘッダー列は処理対象外とし、値または数式が入っていても変更しない。非空ヘッダーが重複している場合は、同じ値を該当するすべての列へ出力する。非空ヘッダーが固定管理列名と同じ場合も、固定管理列と E 列以降の出力列の両方へ出力する。出力行を書き込むときは、A:D の固定管理列と、E 列以降の定義済み出力列だけを書き込み、それ以外の列は既存値や数式を変更しない。E 列以降の既存列は削除せず、出力列を左詰めに正規化しない。新しく増えた出力列は空欄のままとし、既存 OK 行の値を埋める必要がある場合は `Overwrite` または `FillBlankCells` で再取得する。`FillBlankCells` では、有効な出力列に本当に空白のセルがある既存行だけを再取得し、固定管理列と有効な出力列のうち、本当に空白かつ今回の出力値が空文字列ではないセルだけを書き込む。値、数式、または空文字列が入っている既存セルは上書きしない。
...
`SkipExisting` では、`取得状態=OK` の既存 `対象ID` だけをスキップし、`取得状態=ERROR` の既存行は取得エラー行として再取得する。既存行の `取得状態` が `OK` / `ERROR` 以外の場合は、出力シート不整合として中断する。`ダウンロード状態=ERROR` でも `取得状態=OK` の既存行は、`SkipExisting` のスキップ対象とする。`Overwrite` では既存状態に関係なく全更新する。`FillBlankCells` では既存行の `取得状態` に関係なく、有効な出力列に本当に空白のセルがある場合だけ再取得する。有効な出力列に本当に空白のセルがない既存行は、`取得状態=ERROR` であってもスキップ対象とする。未登録の `対象ID` は、どの `ExistingRowMode` でも通常の新規出力行として末尾へ追加する。`RetryError` は旧称であり、settings に指定されている場合は設定誤りとして収集開始前に中断する。`ExistingRowMode` が不正な場合は、指定値と有効値 `SkipExisting`, `Overwrite`, `FillBlankCells` が分かるエラーメッセージを表示する。

## Waiting And Error Handling

画面到達判定は URL より selector を優先する。
詳細ページ到達判定では、詳細遷移操作行の `WaitSelector` の出現を必須条件とする。`TargetIdSelector` は到達判定とは別に、詳細ページ上の対象ID読み取りに使う。一覧上の対象IDと詳細ページ上の対象IDが両方取れる場合、診断モードでは一致を必須とし、不一致は失敗扱いにする。本番収集では不一致を詳細ページ単位の ERROR として出力し、一覧復帰できる場合は次の一覧項目へ進む。
ページング前は `NextPageAvailableSelector` で次ページ操作の有効性を確認する。ページング後は `NextPageOperationName` の操作行の `WaitSelector` の出現を待ったうえで、`TimeoutSeconds` の期限内に一覧先頭の対象IDが前ページの先頭対象IDから変化することを確認する。期限内に変化しない場合は、次ページ操作後にページが進まないものとして本番収集を中断する。ページごとの先頭対象IDを記録し、同じ先頭対象IDが再登場した場合はページングが循環したものとして本番収集を中断する。初回ページの一覧項目が 0 件の場合は収集対象なしとして正常終了する。次ページ操作後に一覧項目が 0 件の場合、または先頭対象IDが読めない場合は、selector 誤り、画面崩れ、認証切れ、ページング失敗の可能性があるため本番収集を中断する。
