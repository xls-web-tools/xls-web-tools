...
- 必須かどうか。
- 空欄時の扱い。
- 詳細ページ列定義は全候補列が空欄の行だけを空行として無視する。
...
条件式で参照できる列は、詳細ページ列定義の `OutputColumnName` に限定する。出力列として選ばれていない抽出列も条件式から参照できる。固定管理列の `対象ID`、`取得状態`、`エラー内容`、`ダウンロード状態` 自体は条件式から参照しない。同名の `OutputColumnName` を持つ詳細ページ列定義がある場合、`[対象ID]` などの列参照は固定管理列ではなく、その抽出列を参照する。
...
詳細ページ列定義で同じ `OutputColumnName` が複数定義されている場合は、`OutputConditionExpression` の有無や output シートの出力列指定に関係なく settings エラーとする。すべての抽出列で `OutputColumnName` は必須とする。`OutputColumnName` が固定管理列名の `対象ID`、`取得状態`、`エラー内容`、`ダウンロード状態` と一致する場合も settings エラーにはしない。
...
診断モードの出力先は本番収集と同じ出力先シートとし、output シートのヘッダーによる列構成も本番収集と同じにする。
