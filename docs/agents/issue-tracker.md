...
- root、`xls-bfw-tools`、`xls-ces-timesheet` は `tkmr-akhs` owner、`xls-web-tools` は `xls-web-tools` owner、`xls-common-devtools`、`DoxyVB6`、`VBA-LanguageServer` は `modern-vba` owner で管理する。GitHub API が許可する範囲では、owner をまたぐ Relationships も設定する。
