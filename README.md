...
`settings` シートでは、A 列に `DetailColumnDefinitions` と書いた行の右側に `OutputColumnName`, `Selector`, `ExtractType`, `AttributeName`, `IsRequired`, `ValueExpression`, `OutputValueType`, `BlankMode` の表を置きます。`output` へ出したい列は、`output` シート 1 行目の E 列以降に `OutputColumnName` と一致するヘッダーとして入力します。
...
`output` シートでは、A:D がツール管理の固定管理列です。A1:D1 の表示名は利用者が変更できます。A1:D1 が本当に空白の場合だけ既定の見出しを書き込み、値、数式、または空文字列が入っている場合は上書きしません。E 列以降は、1 行目に `settings` の `OutputColumnName` と一致する非空ヘッダーを置いた列だけへ抽出値を書き込みます。空白ヘッダー列と未定義ヘッダー列は変更しません。
...
- `output` の A1:D1 は表示名として変更できます。本当に空白のセルだけ既定の見出しで初期化されます。A:D の列位置は固定管理列として使うため、A:D 自体の削除や並べ替えはしないでください。E 列以降は出力列ヘッダーとして使います。
