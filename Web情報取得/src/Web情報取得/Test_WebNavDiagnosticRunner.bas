...
Public Sub Test_WebNavDiagnosticRunner_派生列を出力対象条件で参照して診断行を書く(ByVal Assert As UnitTestAssert)
    On Error Resume Next
    ...
    Set WsSrv = ws_stub
    Call pSetManagedHeadersBlank(ws_stub, False)
    Call pPrepareOutputHeaders(ws_stub, "採用列")
    ...
    tool_settings.TargetIdSelector = "#target-id"
    tool_settings.ReturnToListOperationName = "ReturnToList"
    tool_settings.OutputConditionExpression = "[採用列] == ""対象"""
    ...
    Dim detail_defs As ObjectList
    Set detail_defs = New_ObjectList("DetailColumnDefinition")
    Call detail_defs.Add(New_DetailColumnDefinition("列1", "#primary"))
    Call detail_defs.Add(New_DetailColumnDefinition("列2", "#secondary"))
    ...
    Call client_double.Store.SetReturn("Execute", "{""value"":null}", "POST", "/session/abc/element/detail-link-element/click", "{}")
    Call pSetTextElement(client_double, "#target-id", "target-element", "T-001")
    Call pSetTextElement(client_double, "#primary", "primary-element", "対象")
    Call pSetTextElement(client_double, "#secondary", "secondary-element", "")
    Call client_double.Store.SetReturn("Execute", "{""value"":null}", "POST", "/session/abc/frame", "{""id"":null}")
    Call client_double.Store.SetReturn("Execute", "{""value"":null}", "DELETE", "/session/abc", "")
    ...
    ' --- Assert ---
    If Not Assert.ErrorNotRaised(0, Err.Number, Err.Source, Err.Description) Then Exit Sub
    Assert.IsFalse runner.IsOutputExcluded
    Call pAssertWrittenCell(Assert, ws_stub, 2, 2, G_WEB_STATUS_OK)
    Call pAssertWrittenCell(Assert, ws_stub, 2, 5, "対象")
End Sub

Public Sub Test_WebNavDiagnosticRunner_条件一致後の派生列BlankMode空欄はERROR行にする(ByVal Assert As UnitTestAssert)
    On Error Resume Next
    ...
    Set WsSrv = ws_stub
    Call pSetManagedHeadersBlank(ws_stub, False)
    Call pPrepareOutputHeaders(ws_stub, "派生")
    ...
    Call detail_defs.Add(New_DetailColumnDefinition("派生", "", ValueExpression:="""""", BlankMode:="ErrorIfBlank"))
    Set tool_settings.DetailColumnDefinitions = detail_defs
    ...
    Call client_double.Store.SetReturn("Execute", "{""value"":null}", "POST", "/session/abc/element/detail-link-element/click", "{}")
    Call pSetTextElement(client_double, "#target-id", "target-element", "T-001")
    Call pSetTextElement(client_double, "#decision", "decision-element", "対象")
    Call client_double.Store.SetReturn("Execute", "{""value"":null}", "POST", "/session/abc/frame", "{""id"":null}")
    Call client_double.Store.SetReturn("Execute", "{""value"":null}", "DELETE", "/session/abc", "")
    ...
    Call pAssertWrittenCellContains(Assert, ws_stub, 2, 3, "派生")
End Sub

Public Sub Test_WebNavDiagnosticRunner_条件不一致なら派生列BlankMode空欄でもERROR行にしない(ByVal Assert As UnitTestAssert)
    On Error Resume Next
    ...
    Call client_double.Store.SetReturn("Execute", "{""value"":null}", "POST", "/session/abc/element/detail-link-element/click", "{}")
    Call pSetTextElement(client_double, "#target-id", "target-element", "T-001")
    Call pSetTextElement(client_double, "#decision", "decision-element", "対象外")
    Call client_double.Store.SetReturn("Execute", "{""value"":null}", "POST", "/session/abc/frame", "{""id"":null}")
    Call client_double.Store.SetReturn("Execute", "{""value"":null}", "DELETE", "/session/abc", "")
...
Public Sub Test_WebNavDiagnosticRunner_詳細列定義なしでも固定管理列だけの診断行を書く(ByVal Assert As UnitTestAssert)
    ...
    ' --- Assert ---
    If Not Assert.ErrorNotRaised(0, Err.Number, Err.Source, Err.Description) Then Exit Sub
    Assert.EqualsNumeric 0, ws_stub.Store.GetCallCount("WriteCell", New_RangeBounds(Row:=1, Column:=1, Sheet:="output"))
    Assert.EqualsNumeric 0, ws_stub.Store.GetCallCount("WriteCell", New_RangeBounds(Row:=1, Column:=2, Sheet:="output"))
    Assert.EqualsNumeric 0, ws_stub.Store.GetCallCount("WriteCell", New_RangeBounds(Row:=1, Column:=3, Sheet:="output"))
    Assert.EqualsNumeric 0, ws_stub.Store.GetCallCount("WriteCell", New_RangeBounds(Row:=1, Column:=4, Sheet:="output"))
    Call pAssertWrittenCell(Assert, ws_stub, 2, 1, "T-001")
    Call pAssertWrittenCell(Assert, ws_stub, 2, 2, G_WEB_STATUS_OK)
    Call pAssertWrittenCell(Assert, ws_stub, 2, 3, "")
